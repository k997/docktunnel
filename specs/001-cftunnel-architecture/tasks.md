...
- [X] T087 [P] Create end-to-end integration test in tests/integration/end_to_end_test.go (full lifecycle: container start → tunnel created → container stop → tunnel removed)
...
- [X] T100 Validate 5-second event processing SLA with timing benchmarks in controller event handler
...
- [X] T103 Add API token validation on startup in internal/config/config.go (test Cloudflare API call, fail fast if invalid)
- [X] T104 Ensure API tokens and secrets never appear in logs in internal/logger/logger.go (sanitize log output)
